...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname'])){
			...
			$errors[] = "Empty Surnames";
		}  elseif (empty($_POST['user_name'])) {
            $errors[] = "Username empty";
        } elseif (empty($_POST['user_password_new']) || empty($_POST['user_password_repeat'])) {
            $errors[] = "Empty password";
        } elseif ($_POST['user_password_new'] !== $_POST['user_password_repeat']) {
            $errors[] = "the password and the repetition of the password are not the same";
            ...
            $errors[] = "Username does not fit in the name scheme: Only aZ and numbers are allowed, from 2 to 64 characters";
        } elseif (empty($_POST['user_email'])) {
            $errors[] = "The email can not be empty";
        } elseif (strlen($_POST['user_email']) > 64) {
            $errors[] = "The email can not be longer than 64 characters";
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
                ...
                if ($query_check_user == 1) {
                    $errors[] = "Sorry, the username or email address is already in use.";
                } else {
                    ...
                    if ($query_new_user_insert) {
                        $messages[] = "The account has been created successfully.";
                    } else {
